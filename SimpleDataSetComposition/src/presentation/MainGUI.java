	...
	public MainGUI()
	  {
	    ...
	    OutputManager.writeToFile("WARNING!\nTHE FULL VERSION OF THE ALGORITHM IS IN ANOTHER PROJECT. \nThis project only runs the sample data from the project description. \nPlease run to the PlannerFull project to test the algorithm on the actual challenge files.");
	  }
			  ...
			  int solutionsWanted = Integer.valueOf(mSolutionCombo.getSelectedItem().toString());
			    if (solutionsWanted > 0)
			    {
			    	try
			    	{
			    		...
			    		Driver.run("N/A", "N/A", "N/A", solutionsWanted, 1);
			    	}
			    	catch (Exception e)
			    	{
...
private String owlFile = "";
  ...
  protected JButton mButtonDoIt = new JButton("[ RUN SAMPLE SEARCH ]");
